...
client.on("guildMemberAdd", function(member) {
    member.guild.channels.find("name", "main").sendMessage(member.toString() + " Welcome to the Discord, Darlin. :kiss:");
...
client.on("message", function(message) { 
    ...
    var mutedrole = message.guild.roles.find("name", "Muted");
    var mutedrole = message.guild.roles.find("name", "Mute");
    var mutedrole = message.guild.roles.find("name", "mute");
    ...
    if (command == "help") { 
        var embedhelpmember = new Discord.RichEmbed() 
            ...
            .addField(" - cookie", "Sends a friendship cookie to a user! (Correct usage: .cookie @user") 
            .setColor(0xFFA500) 
    ...
    if (command == "helpop") {
        var embedhelpadmin = new Discord.RichEmbed()
            ...
            .setColor(0xFF0000) 
            .setFooter("Mmmm, an admin!")
        message.channel.send(embedhelpmember); 
        if(message.member.roles.some(r=>["bot-admin"].includes(r.name)) ) return message.channel.send(embedhelpadmin); 
    }

    if (command == "info") {
        message.channel.send("Hey! My name is **AniBot** and I'm here to assist you! You can do ***.help*** to see all of my commands! :smile:")
    }
